...
import xiangshan.backend.Bundles.{ExuInput, ExuOutput, IssueQueueIQWakeUpBundle, MemExuOutput}
...
class MemExeUnitIO(exuParams: ExeUnitParams)(implicit p: Parameters) extends XSBundle {
  val flush = Flipped(ValidIO(new Redirect()))
  val in = Flipped(DecoupledIO(new ExuInput(exuParams, hasCopySrc = true)))
  val out = DecoupledIO(new MemExuOutput())
}

class MemExeUnit(exuParams: ExeUnitParams)(implicit p: Parameters) extends XSModule {
  val io = IO(new MemExeUnitIO(exuParams))
  val fu = exuParams.fuConfigs.head.fuGen(p, exuParams.fuConfigs.head)
  fu.io.flush             := io.flush
  fu.io.in.valid          := io.in.valid
  io.in.ready             := fu.io.in.ready

  fu.io.in.bits.ctrl.robIdx    := io.in.bits.robIdx
  fu.io.in.bits.ctrl.pdest     := io.in.bits.pdest
  fu.io.in.bits.ctrl.fuOpType  := io.in.bits.fuOpType
  fu.io.in.bits.data.imm       := io.in.bits.imm
  fu.io.in.bits.data.src.zip(io.in.bits.src).foreach(x => x._1 := x._2)
  fu.io.in.bits.perfDebugInfo := io.in.bits.perfDebugInfo
  fu.io.in.bits.debug_seqNum := io.in.bits.debug_seqNum
  ...
  io.out.bits.uop.fuType  := io.in.bits.fuType
  io.out.bits.uop.fuOpType:= io.in.bits.fuOpType
  io.out.bits.uop.sqIdx   := io.in.bits.sqIdx.get
